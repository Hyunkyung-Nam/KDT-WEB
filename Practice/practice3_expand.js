let date, wantedMonth, wantedYear;
...
$('.inputBtnSection').click(function () {
    ...
    const inputCalenderContent = `<p style="font-size:14px;color:black;text-align:left;">* ${content.val()}</p>`;

    $(`.day${date.text()}`).append(inputCalenderContent);
    // console.log($(`.day${date.text()}`).children());
    //$(`.day${date.text()}`).children().css({ 'font-size': '14px', color: 'black', 'text-align': 'left' });
    content.val('');
    $('.popUpCalenderContent').addClass('isHide');
});

$('td').click(function () {
    //아이디가 없거나 첫번ㅈ
    date = $(this);
    if (date.text() < 32 && date.text() > 0) {
        $('.popUpCalenderContent').children('div:eq(0)').text(`${wantedMonth}월 ${date.text()}일`);
        $('.popUpCalenderContent').removeClass('isHide');
    }
});
